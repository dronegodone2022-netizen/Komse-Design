...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const normalizedItems = items.map((item: { productId?: string; quantity?: number; selectedSize?: string; selectedColor?: string }) => {
      ...
      return { productId: item.productId || '', name: product?.name || '', quantity: Math.max(1, Math.floor(item.quantity || 0)), priceEur: product?.priceEur || 0, inStock: product?.inStock === true, selectedSize: item.selectedSize || '', selectedColor: item.selectedColor || '' };
    });
    ...
    const session = await stripe.checkout.sessions.create({ mode: 'payment', line_items: lineItems, customer: stripeCustomer.id, billing_address_collection: 'required', phone_number_collection: { enabled: true }, shipping_address_collection: { allowed_countries: ['FR', 'GB', 'US', 'SL', 'SN', 'CI', 'GH', 'NG'] }, success_url: `${appUrl()}/?payment=success&session_id={CHECKOUT_SESSION_ID}`, cancel_url: `${appUrl()}/?payment=cancelled`, metadata: { orderId: body.orderId, userId: user.id, customerName: customer.name || 'Customer', subtotalEur: subtotalEur.toFixed(2), shippingEur: shippingEur.toFixed(2), itemsCount: String(normalizedItems.reduce((count, item) => count + item.quantity, 0)), itemsSummary: normalizedItems.map((item) => `${item.quantity}x ${item.name}`).join(', '), itemsData: JSON.stringify(normalizedItems.map(({ productId, name, quantity, selectedSize, selectedColor }) => ({ productId, name, quantity, selectedSize, selectedColor }))) } });
