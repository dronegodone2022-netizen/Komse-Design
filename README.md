...
GitHub Actions validates and deploys the frontend, but it cannot run an interactive Stripe checkout or host the payment webhook. For end-to-end payment testing, deploy the Node backend to a Node.js host, configure its server-only environment variables, and point the GitHub Actions `VITE_API_URL` variable to that backend. Update Stripe's webhook endpoint and `APP_URL` to the final public URLs.
...
The payment, contact email, and admin order update flows use Edge Functions in `supabase/functions/stripe-checkout`, `supabase/functions/stripe-webhook`, `supabase/functions/contact-email`, and `supabase/functions/admin-order-update`. Deploy them with the Supabase CLI, set the function secrets (`STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `SUPABASE_SERVICE_ROLE_KEY`, `APP_URL`, `RESEND_API_KEY`, `MAIL_FROM`, and `ADMIN_EMAIL`), apply `supabase-migration.sql`, and configure Stripe's webhook URL as `https://YOUR_PROJECT.supabase.co/functions/v1/stripe-webhook`. Set `VITE_SUPABASE_FUNCTIONS_URL` to `https://YOUR_PROJECT.supabase.co/functions/v1` or let the frontend derive it from `VITE_SUPABASE_URL`.
